...
import voxpop
import vpNLP as vpNLP
import vpStats as vpStats
...
class Item():
	...
	def load_doc(self):
		...
		try:
			with voxpop.VPE.db_lock:
				_doc = json.loads(voxpop.VPE.get_db().open_document(self._id))
	...
	def load_children(self):
		...
		try:
			with voxpop.VPE.db_lock:
				cached_couch = json.loads(voxpop.VPE.get_db().open_document('_design/children/_view/for_key',key='"'+self._id+'"',include_docs='true'))
	...
	def load_descendents(self):
		...
		try:
			with voxpop.VPE.db_lock:
				cached_couch = json.loads(voxpop.VPE.get_db().open_document('_design/descendents/_view/for_key',key='"'+self._id+'"',include_docs='true'))
			if 'rows' in cached_couch:
				for i in cached_couch['rows']:
	...
	def run_stats_on_me(self,**kwargs):
		...
		if self.stats_jid is None:
			...
			with voxpop.VPE.beanstalkd_lock:
				...
				self.stats_jid = voxpop.VPE.get_beanstalkd().put(pickle.dumps(_req),delay=5,pri=100000)
	...
	def persist_me(self):
		...
		if self.save_jid is None:
			...
			with voxpop.VPE.beanstalkd_lock:
				voxpop.VPE.get_beanstalkd().use(self.cache_worker)
				self.save_jid = voxpop.VPE.get_beanstalkd().put(pickle.dumps({'item_id': self._id, 'functions': [persist_item]}), delay=5, pri=_mypri)
...
def run_stats_on_item(message={}):
	...
	item = voxpop.VPE.get_items()[message['item_id']]
	if item is None:
...
def save_item(_item=None):
	...
	def __resolve(_id):
		...
		with voxpop.VPE.db_lock:
			couched = json.loads(voxpop.VPE.get_db().open_document(_id))
		...
		try:
			...
			with voxpop.VPE.db_lock:
				resp = voxpop.VPE.get_db().save_document(_save_doc,_id)
			return True
	...
	try:
		...
		with voxpop.VPE.db_lock:
			response = json.loads(voxpop.VPE.get_db().save_document(_myItem,_item._id))
		del _myItem
		if 'rev' in response:
...
def bulk_save_items(cache=None):
	...
	def __resolve(_id):
		...
		try:
			with voxpop.VPE.db_lock:
				resp = voxpop.VPE.get_db().save_document(cache[i['id']],_id)
			return True
		except CouchConflict:
			__resolve(_id)
			return False
	...
	if len(_docs) == 0:
		...
		return True
	with voxpop.VPE.db_lock:
		resp = voxpop.VPE.get_db().bulk_save_documents(_docs)
	if resp is False:
		return False
	for i in resp:
