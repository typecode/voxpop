...
def fetch_articles_for_query(query_text=None,**kwargs):
	...
	if 'query_id' in kwargs:
		query = voxpop.VoxPopEnvironment.get_items().get(_id=kwargs['query_id'])
	else:
...
def get_new_articles_for_query(query):
	...
	for i in range(0,n_requests):
		...
		with voxpop.VoxPopEnvironment.beanstalkd_lock:
			voxpop.VoxPopEnvironment.get_beanstalkd().use("nytarticle")
...
def cache_response(message={}):
	...
	if 'query_id' in message:
		query = voxpop.VoxPopEnvironment.get_items().get(_id=message['query_id'])
		query.init_stats = True
	for i in message['json'][u'results']:
		...
		article = voxpop.VoxPopEnvironment.get_items().get(_id=article_id)
		...
		if u'des_facet' in i:
			for f in i[u'des_facet']:
				_f = voxpop.VoxPopEnvironment.get_items().get(pars={'kind':'des_facet', 'name':f})
				_f.init_stats = True
				_f.add_child(article_id)
				article.add_to_cache(_f._id,False)
		if u'per_facet' in i:
			for f in i[u'per_facet']:
				_f = voxpop.VoxPopEnvironment.get_items().get(pars={'kind':'per_facet', 'name':f})
				_f.init_stats = True
				_f.add_child(article_id)
				article.add_to_cache(_f._id,False)
		if u'geo_facet' in i:
			for f in i[u'geo_facet']:
				_f = voxpop.VoxPopEnvironment.get_items().get(pars={'kind':'geo_facet', 'name':f})
				_f.init_stats = True
				_f.add_child(article_id)
				article.add_to_cache(_f._id,False)
		if u'org_facet' in i:
			for f in i[u'org_facet']:
				_f = voxpop.VoxPopEnvironment.get_items().get(pars={'kind':'org_facet', 'name':f})
				_f.init_stats = True
				_f.add_child(article_id)
				article.add_to_cache(_f._id,False)
