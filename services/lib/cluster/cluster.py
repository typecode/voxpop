...
def VPcentroid(list, method=median):
   "returns the central vector of a list of vectors"
   out = {}
   out['timestamp'] = ( median( [x['timestamp'] for x in list] ) )
   out['ratio'] = ( median( [x['ratio'] for x in list] ) )
   return out
...
available. You supplied %d items, and asked for %d clusters.""" %
   ...
   def assign_item(self, item, origin):
      ...
      for cluster in self.__clusters:
         if self.distance(item, VPcentroid(cluster)) < self.distance(item, VPcentroid(closest_cluster)):
