...
function Chart (options) {
	...
	function MouseTracker (chart, options) {
		...
		*/
		 ...
		 */
		function onmousemove (e) {  //FUNCTION MODIFIED BY AMAHON
...
Series.prototype = {
	...
	createArea: function() {
		...
		each(coordsArray, function(coords) {
			...
			/* added by amahon@gmail.com */
			var _coords = coords[0]
			if(isCircle){
				if(coords[1].marker.radius){
					_coords = _coords + ',' + coords[1].marker.radius
				} else {
					_coords = _coords + ',' + chart.options.tooltip.snap
				}
			}
			...
			area = createElement('area', {
				...
				onmouseover: function(e) {
					...
					if(!series.options.enableMouseTracking){
						return
					}
					
					vp.events.trigger('timelinePointMouseover',coords[1])
				...
				onmouseout: function(e) {
					vp.events.trigger('timelinePointMouseout')
